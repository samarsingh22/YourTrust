...
export default function BorrowerPage() {
  ...
  const fetchAgreements = async (userId: string) => {
    try {
      ...
      if (res.ok) {
        const borrowed = (data.agreements || []).filter(
          (a: any) => a.borrowerId === userId
        )
        setAgreements(borrowed)
      }
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 lg:px-6">
      ...
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-bold">Borrowed Agreements</h1>
          <p className="text-sm text-muted-foreground">
            Money & assets you&apos;ve borrowed
          </p>
        </div>
      </div>
      ...
      <div className="mb-6 rounded-2xl border border-border bg-card p-5">
        <div className="flex items-center gap-3 text-orange mb-1">
          ...
          <span className="text-sm font-medium text-muted-foreground">Total Borrowed (Active)</span>
        </div>
      ...
      <div className="space-y-3">
        {agreements.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 rounded-xl border border-border bg-card">
            <ArrowDownLeft className="h-10 w-10 text-muted-foreground mb-3" />
            <h3 className="font-semibold mb-1">No Borrowed Agreements</h3>
            <p className="text-sm text-muted-foreground mb-4">
              You haven&apos;t borrowed any money or assets yet
            </p>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-orange/20 text-orange font-bold text-sm">
                      {initials(agreement.lenderName)}
                    </div>
                    <div>
                      <p className="font-semibold text-sm">From {agreement.lenderName}</p>
